...
- **Invariants.** Enforced in-step (clamps per R-52) and re-validated post-step; violation ⇒ deterministic `InvalidStateError` (no silent repair): NaN/∞ rejected, bounds checked, divergence guard (any stock moving > 50 index/tick for 3 consecutive ticks without an intervention flags a kernel bug in dev/test). **Exemption (review A finding 10):** the guard does **not** fire on rule-driven, clamped, rule-ID-stamped moves — an R-01b bloom die-back and an R-40 stress die-off are lawful, bounded, single-tick crashes, not divergence. The guard therefore evaluates the *unclamped* pre-clamp delta, exempts any move whose flow ledger carries a rule ID and whose post-clamp value lies inside [0, cap], and raises its threshold to 50 for every non-crash stock. A legal crash must never be reported as a kernel bug, or the guard becomes a mute button.
...
9. **Per-tick events in the trace (review A finding 15 → D-3.4).** Each tick's canonical JSON record carries an `events` array **with a fixed key order** (`rule`, `kind`, `organism?`, `id?`, `value?`), appended in the frozen §6.5 evaluation order and capped at 64 per tick (a cap, not a drop: overflow is a kernel error, not a silent truncation). Events are evidence-layer *inputs*: they are serialized so the evidence UI can be rebuilt from a trace alone, and they are explicitly non-authoritative — no rule may read them (D-2 line 41). Traces without `events` stay replayable; traces with them are the evidence artifact.
10. **Auto-run interruption (review A finding 15 → D-3.5).** When `autoRun(days)` yields early (player input, intervention, focus loss), the recorded action is the *exact* completed `advanceDays(n)` for the ticks already committed — never the requested span. Interrupting at tick k of n with an intervention records `advanceDays(k − start)`, then the intervention action at tick k; the replay therefore reconstructs the same trace whether or not the player interrupted. The same rule applies to the UI's notion of "days elapsed".
