...
| | |
| --- | --- |
| Status | **FROZEN STRUCTURE, PROVISIONAL VALUES.** The rules below are frozen as `pond-crisis-1.2` (GAME-317 decision (d), §9.1 F-2/F-3). The numeric set `pc1-params-1.2` in §9 is **provisional**: the evidence harness passes 27 of 45 checks and the §11-2 canonical chain does not yet land inside every window. Nothing here may be presented as verified science. See the banner and §9.1. |
...
| Model version identity | `SIM_MODEL_VERSION = "pond-crisis-1.2"`, `PARAM_SET_VERSION = "pc1-params-1.2"` (emitted in every replay trace; matches `evidence/calibration-sim.mjs` exactly) |
...
> 3. **`SIM_MODEL_VERSION` / `PARAM_SET_VERSION` are bumped to `pond-crisis-1.2` / `pc1-params-1.2`** and now agree with `evidence/calibration-sim.mjs` — the 1.0/1.1 mismatch between this document and the harness is fixed.
> 4. **`p.nutrientHalfSaturation` is added** (R-01's supply factor) and `p.bloomCrashThreshold` is retired from the frozen rule.
>
> **What is NOT done.** The harness passes 27 of 45 checks. The canonical chain does not yet land inside all of its §11-2 windows, the pristine pond's DO is 5.4 mg/L (below the healthy pond this document describes), and the pool still pins at its cap. The values in §9 are therefore **provisional**, and §9.1 lists the seven findings (F-1…F-7) and the seven explicit ER-04 gate items. **ER-03 must not begin until the harness is green** and §9 is re-frozen with a Jira record; GAME-317's acceptance criteria are not all met by this revision.
...
- **R-10 Per-link removal (Holling-II shape).** Removal is **directly parameterized per link**: for consumer C eating prey X, `removal = min( p.removalRate[C] × h(X) × stock[C], preyAvailable[X] )`, with `h(X) = X/(p.halfSaturation + X)` for the filter-feeding grazers (water fleas, mayflies, snails) and `h(X) = X/(p.predationHalfSaturation + X)` for the predators (bluegill, dragonfly nymphs) — **two** half-saturations, because a predator's functional response saturates at a different prey density than a filter-feeder's, and one shared value cannot both keep grazing near saturation at bloom density and stop a predator from over-taking sparse prey (finding F-3). The `min(…, preyAvailable)` term is the mass guard: a link can never remove more prey than exists, and the frozen evaluation order (§6.5) decides who eats first when several links share a prey. *Source:* standard functional-response form; the split follows S10 (filter feeding) and S13/S14 (predator intake). *Simplification:* no prey switching, no refuges, no handling time.
...
| p.algaeSenescence | 0.028 | 0.008–0.08 | R-04 | Fraction of algae → detritus /tick |
| p.weedSenescence | 0.050 | 0.008–0.05 | R-04 | Fraction of weeds → detritus /tick |
...
| p.halfSaturation (grazing) | 28 | 3–40 | R-10 | Algal index at half grazer intake. **Not a free knob**: a low value destabilizes the pristine equilibrium (§9.1 F-6) |
| p.predationHalfSaturation | 56 | 8–80 | R-10 | Prey index at half predator intake (separate from grazing — F-3) |
| p.removalRate | flea 0.030 · mayfly 0.042 · snail 0.030 · bluegill 0.010 · dragonfly 0.010 | 0.004–0.30 (grazers); 0.002–0.05 (predators) | R-10 | Per-link removal at saturation; replaces v1.0's `p.maxIntake` |
| p.egestionFraction | 0.096 | 0–0.40 | R-20 | Share of removed prey mass egested → detritus |
| p.exportFraction | 0.070 | 0.05–0.50 | R-20 | Share of consumer mortality + egestion leaving the pond |
| p.maintenance | 0.1372 | 0.02–0.20 | R-12 | Maintenance respiration as a share of **assimilated intake** (F-1) |
| p.carryingCapacity | flea 71 · mayfly 61 · snail 65 · bluegill 84 · dragonfly 42 | 2–400 (per species) | R-12 | Recruitment cap. Sets where each species' births stop paying for its deaths |
| p.backgroundMortality | 0.006 | 0.004–0.04 | R-12 | /tick |
| p.decompRate | 0.40 | 0.15–0.40 | R-21 | Fraction of detritus decomposed/tick (at the range top) |
| p.mineralizationFraction | 0.95 | 0.30–0.95 | R-21 | Share of decomposed matter returned to the pool; the remainder is buried. **Load-bearing**: the return flux (this × decompRate × detritus) is what decides whether the pool can ever fall (§9.1 F-6) |
| p.backgroundInflow | 0.344 /tick | 0–3 (solved) | R-30 | Undisturbed watershed supply; **solved** per parameter set so the pool rests on the reference, never hand-set |
| p.nutrientSinkRate | 0.0202 | 0.02–0.10 | R-30 | Settling/denitrification on the pool's excess over the reference |
| p.nutrientReference | 21.0 | 0.5–40 (per scenario) | R-30 | The pristine pool level — a **per-scenario anchor**, not a universal 50 |
...
| p.o2PerPhotosynthesis | 0.00048 | 0.002–0.008 declared; 0–0.004 enumerated | R-22 | Per unit algae/tick, capped by `photoCap` 0.5. **Range conflict — see F-6 note** |
...
| p.stressMortality | 0.20 /tick | 0.05–0.20 | R-40 | Max per-tick mortality at the severe threshold |
...
identity. **Current status: 27 of 45 checks pass.** The pristine pond is a genuine stationary fixed
...
- **Pristine pond (`§11-1`, strict): PASS.** The undisturbed fixed point is `{nutrients 21.0, algae 53.0, weeds 49.4, flea 30.1, mayfly 21.7, snail 27.0, bluegill 16.3, dragonfly 8.1, sediment 14.0, DO 5.40}` with 60-tick drift ≈ 0.00 on every stock. It is produced by the settle (the watershed inflow is controlled onto the reference), never hand-set.
...
1. **Make the canonical chain land inside the §11-2 windows**, starting from the F-8/F-9 diagnosis rather than F-6's: the die-back fires correctly but too slowly, because the grazer community that supplies its main loss term collapses through competitive exclusion. **F-9 needs a decision before it needs a search** — the three grazers are identical in the frozen rules, so either R-10 gains a per-link grazing half-saturation (the same move that F-3 already made for predation) or R-14 gains an explicit coexistence mechanism. That is a rule-structure decision at this gate's level, not an ER-04 tuning choice. The harness is the acceptance test and must be green before ER-03 calibrates a kernel against it.
...
| F-9 | **The three grazers are ecologically identical in the frozen rules, so the model competes them to exclusion.** All three eat the same pool through the same Holling form with the same half-saturation; only `p.removalRate` and `p.carryingCapacity` can differ, and both are parameters rather than mechanisms. Measured repeatedly during the settle: one grazer reaches its own level and the other two fall to zero, taking the grazing that F-8 needs with them. A low `p.predationHalfSaturation` (the saturating-predator mechanism that would stabilise coexistence by concentrating mortality on the abundant prey) did **not** fix it at the predator abundances the provisional set sustains. | This is the single largest blocker and it is a **rule-structure** question, not a tuning one: either the grazers need differentiated functional responses (a half-saturation *per link*, the same reasoning that already justifies F-3's separate predation half-saturation), or the model needs an explicit coexistence mechanism (refuges, a per-species defence, or a predation response that switches). ER-04 must settle this before it can close F-8, because a missing grazer removes the die-back's main loss term. |
