...
1. **Make the canonical chain land inside the §11-2 windows**, starting from the diagnosis in F-6: shape the return flux so a starved bloom actually sheds its standing crop in a few ticks. The harness is the acceptance test and must be green before ER-03 calibrates a kernel against it.
...
3. **Resolve the pristine-DO/trough tension** in `p.reAeration` (or by another declared route): the undisturbed pond must be ≥ ~6.9 mg/L *and* the canonical crash must still pull DO below 5.0.
