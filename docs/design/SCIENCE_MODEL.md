...
| Model version identity | `SIM_MODEL_VERSION = "pond-crisis-1.4"`, `PARAM_SET_VERSION = "pc1-params-1.4"` (emitted in every replay trace; matches `evidence/calibration-sim.mjs` exactly) |
...
- **R-21 Decomposition & oxygen consumption.** `decomposition = p.decompRate × detritus × tempFactor`; `oxygenDemand = p.o2PerDecomposition × decomposition + p.o2PerBurst × detritusInflow`; `mineralizationFraction` of the decomposed matter returns to the nutrient pool and the remainder is **buried** (leaves the active cycle). The two oxygen terms are the substance of finding F-13 and they are not redundant: the first is the standing sediment's ongoing draw, the second is the **BOD pulse** from freshly dead matter — a newly collapsed bloom is labile and is consumed far faster than the aged mud beneath it, which is why a real pond's oxygen sags when a bloom dies rather than in proportion to the sediment already present. The second term is what makes the canonical trough reachable at all: with the demand keyed to the standing stock alone, the pristine and post-crash stocks differ by less than a factor of two, so no sensitivity both keeps the undisturbed pond oxygenated and pulls the canonical run below 5 mg/L. `tempFactor` is frozen at 1.0 in v1 (temperature is a per-scenario constant, §3), so it is a placeholder for a future temperature rule, not a live parameter. Decomposer activity index displayed = normalized decomposition rate (**normalization is an ER-04 gate item** — see §9.1's ER-04 list). The decomposition pipeline plus the detritus stock is what produces the canonical **2–4 tick lag** between the bloom's crash and the DO trough. *Source:* S2 ("when the algae die, they are decomposed by bacteria — this process consumes the oxygen dissolved in the water"), S3, S18 (BOD: microbial decomposition of organic matter consumes dissolved oxygen).
...
| p.o2PerDecomposition | 0.03 | 0.01–0.04 | R-21 | O2 cost per unit of decomposed matter — the standing sediment's ongoing draw |
| p.o2PerBurst | 0.045 | 0.01–0.10 | R-21 | **New in 1.4.** O2 cost per unit of FRESH dead matter arriving this tick — the BOD pulse. This is what makes the canonical oxygen trough reachable at all (F-13): with the demand a function of the standing stock alone, the pristine and post-crash stocks differ by less than 2×, so no sensitivity both keeps the pristine pond healthy and pulls oxygen below 5 |
| p.reAeration | 0.10 | 0.05–0.25 | R-22 | Relaxation toward saturation; aeration substitutes 0.30. **The pristine-DO/trough conflict lives here** (§9.1 F-3) |
